...
export default function App() {
  ...
  const loadImageFromUrl = useCallback(async (rawValue: string) => {
    ...
    try {
      ...
      setImageError(
        "Unable to load this image URL. Use a direct image link or upload the file instead.",
      );
    } finally {
    ...
    if (file) {
      void (async () => {
        ...
        try {
          ...
          setImageError(
            "Unable to read the selected file. Please try another image.",
          );
        } finally {
          setIsImportingImage(false);
        }
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      alert(
        "Failed to generate mockup. If you are using an external image URL, try uploading the image file instead due to browser security restrictions.",
      );
    } finally {
  ...
  const headerHeight =
    frameType === "mac" ? 40 : frameType === "chrome" ? 80 : 0;
  const maxImgHeight = canvasDims
    ? Math.max(0, canvasDims.height - padding * 2 - headerHeight)
    : undefined;
          ...
          >
            {sizePresets.map((preset) => (
              <option key={preset.id} value={preset.id}>
                {preset.label}{" "}
                {preset.width ? `(${preset.width}x${preset.height})` : ""}
              </option>
            ))}
          </select>
              ...
              style={{
                ...
                width:
                  sizePreset === "auto"
                    ...
                    : sizePreset === "custom"
                      ? customSize.width
                      : sizePresets.find((p) => p.id === sizePreset)?.width,
                height:
                  sizePreset === "auto"
                    ? "auto"
                    : sizePreset === "custom"
                      ? customSize.height
                      : sizePresets.find((p) => p.id === sizePreset)?.height,
                ...
                >
                  <img
                    ...
                    onError={() => {
                      setImage(null);
                      setImageError(
                        "This image could not be rendered. Try uploading the file instead.",
                      );
                    }}
                    style={{
                      borderRadius:
                        frameType === "none" ? `${borderRadius}px` : "0",
